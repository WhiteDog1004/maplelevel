      ...
      <div className='flex flex-col px-8 py-6 gap-4 rounded-lg relative border-2 dark:border-0 bg-zinc-100 dark:bg-zinc-800 hover:-translate-y-2 transition-transform cursor-pointer'>
        <div className='flex gap-4 items-center justify-between'>
          ...
          <div className='flex flex-col text-end gap-4 min-w-24'>
            <div className='flex flex-col gap-1'>
              <div className='flex gap-1 items-center justify-end'>
                <Image
                  ...
                  height={20}
                  unoptimized
                  src={`/images/class/${getClassImages(getLabelByJobs(data.job)?.id || 0)}.webp`}
                  alt='class'
                />
            ...
            <span className={`${getBadgeType[data.hunt_type as ExchangeTypes]} justify-center`}>
              <Typography variant='body2'>
        ...
        <div className='flex flex-col items-center gap-2'>
          <div className='flex flex-row items-center gap-1'>
            ...
            ~
            <Typography variant='caption' sx={{ lineHeight: 1 }} color='textDisabled'>
              {getTextByCode(Number(highestMap.map))?.kor?.split(':')[1]}
            </Typography>
          </div>
        </div>
        <div className='flex justify-center items-center gap-2'>
          <Typography className='dark:text-zinc-500 text-zinc-400' variant='caption'>
            작성자 :
          </Typography>
          <Chip
            color='primary'
